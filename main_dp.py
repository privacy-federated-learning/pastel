...
def fl_training_ppm(args, device):
    ...
    for epoch in tqdm(range(args.epochs)):
        ...
        save_model(args, global_weights, epoch)
        sigma = compute_noise(1, args.q, args.eps, args.E * args.tot_T, args.delta, 1e-5)
        for name in global_weights.keys():
            global_weights[name] += gaussian_noise(global_weights[name].shape, args.clip, sigma, device=device)
...
def ppm_eval(t_train_loader, t_test_loader, t_val_loader, s_train_loader, s_test_loader, s_val_loader, args, model, epoch):
    if args.ppm in config.PASTEL_CONFIG:
        model = pastel(model, args.pastel_layers)
    return create_attack(t_train_loader, t_test_loader, t_val_loader, s_train_loader, s_test_loader, s_val_loader, args, model, epoch)
