...
class Opacus_LocalUpdate(object):
    ...
    def update_weights(self, model, global_round, model_replacement=False, attack=None):
        ...
        no_noise_model = copy.deepcopy(model)
        no_noise_optimizer = copy.deepcopy(optimizer)
        ...
        sigma = compute_noise(len(self.train_loader.dataset), self.args.batch_size, self.args.eps, self.args.E * self.args.tot_T, 0.00001, 0.1)
        ...
        no_noise_model = no_noise_model.to('cuda')
        ...
        for iter in range(self.args.local_ep):
            ...
            for batch_idx, (images, labels) in enumerate(self.train_loader):
                ...
                if attack:
                    images, labels = self.alter_data_set(images, labels)
                no_noise_model.zero_grad()
                # print(images.shape)

                log_probs = no_noise_model(images)
                if not (len(log_probs.shape) == 2 and log_probs.shape[0] == 1):
                    log_probs = log_probs.squeeze()
                loss = self.criterion(log_probs, labels)
                ...
                no_noise_optimizer.step()
                no_noise_optimizer.zero_grad()
                # @Rania : Added this condition
                if attack and self.args.pgd:
                    x_adv = copy.deepcopy(no_noise_model.state_dict())
                    for key in x_adv.keys():
                        x_adv[key] = torch.max(torch.min(x_adv[key], x[key] + eps), x[key] - eps)
                    no_noise_model.load_state_dict(x_adv)
                if self.args.verbose and (batch_idx % 10 == 0):
                    print('| Global Round : {} | Local Epoch : {} | [{}/{} ({:.0f}%)]\tLoss: {:.6f}'.format(
                        global_round, iter, batch_idx * len(images),
                        len(self.train_loader.dataset),
            ...
            epoch_loss.append(sum(batch_loss) / len(batch_loss))
            accuracy, _ = self.test_inference(copy.deepcopy(no_noise_model), self.test_loader)
            # print("Accuracy : ",accuracy)
            accuracies.append(accuracy)

        if model_replacement:
            return model_replacement(no_noise_model.state_dict(), x, self.args.num_users, self.args), sum(epoch_loss) / len(
                epoch_loss)
        else:
            no_noise_state_dict = copy.deepcopy(no_noise_model).state_dict()
            no_noise_state_dict_cp = copy.deepcopy(no_noise_model).state_dict()
            for key in no_noise_state_dict_cp.keys():
                new_key = key.replace('_module.', '')
                no_noise_state_dict[new_key] = no_noise_state_dict.pop(key)

        for key in no_noise_state_dict.keys():
            if self.args.pastel_layers[0] in key:
                print(key)
                no_noise_state_dict[key] = state_dict[key]
        return no_noise_state_dict, sum(epoch_loss) / len(epoch_loss), accuracies, epoch_loss
