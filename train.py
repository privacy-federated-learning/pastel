...
from compression import TensorPruner
...
from dpsgd import DPSGD
#from pyvacy.pyvacy.optim import DPSGD
from tensorflow_privacy.privacy.analysis.compute_noise_from_budget_lib import compute_noise
...
class LocalUpdate(object):
    ...
    def obfuscate_weights(self, w, model, global_round, model_replacement=False, attack=None):
        w = copy.deepcopy(w)
        ...
        elif self.args.optimizer == 'adgd':
            optimizer = Adsgd(model.parameters(), lr=self.args.lr,
                              weight_decay=1e-4)
        ...
        sigma = compute_noise(len(self.train_loader.dataset), self.args.batch_size, self.args.eps,
                              self.args.E * self.args.tot_T, 0.00001, 0.1)
        ...
        for iter in range(self.args.local_ep):
            ...
            for batch_idx, (images, labels) in enumerate(self.train_loader):
                ...
                if not (len(log_probs.shape) == 2 and log_probs.shape[0] == 1):
        ...
        if model_replacement:
            ...
            state_dict_cp = copy.deepcopy(model).state_dict()
            for key in state_dict_cp.keys():
                new_key = key.replace('_module.', '')
                state_dict[new_key] = state_dict.pop(key)
        for key in state_dict.keys():
            if self.args.pastel_layers[0] in key:
                print(key)
    ...
    def update_weights(self, model, global_round, model_replacement=False, attack=None):
        ...
        if self.args.ppm == 'pastel':
            return self.update_weights_pastel(model, global_round, model_replacement=False, attack=None)
        model.train()
        epoch_loss = []
        accuracies = []
        eps = self.args.eps
        ...
        elif self.args.optimizer == 'dp_sgd':

            optimizer = DPSGD(model.parameters(), lr=self.args.lr)
        ...
        for iter in range(self.args.local_ep):
            ...
            for batch_idx, (images, labels) in enumerate(self.train_loader):
                ...
                log_probs = model(images)
                # Apply TensorPruner to gradient tensors
                ...
                batch_loss.append(loss.item())
                if self.args.ppm == 'gc':
                    tensor_pruner = compression.TensorPruner(zip_percent=self.args.gc_preserved_percent)
                    for param in model.parameters():
                        param = tensor_pruner.prune_tensor(param)
                # print(batch_loss)
        ...
        if model_replacement:
            ...
            return copy.deepcopy(model).state_dict(), sum(epoch_loss) / len(epoch_loss), accuracies, epoch_loss

    def update_weights_pastel(self, model, global_round, model_replacement=False, attack=None):
        ...
        for iter in range(self.args.local_ep):
            ...
            for batch_idx, (images, labels) in enumerate(self.train_loader):
                ...
                # Calculate gradients
                loss.backward()
                train_grad = [param.grad.detach().clone() for param in model.parameters()]
                optimizer.step()
                optimizer.zero_grad()

                # Calculate gradients for validation data
                train_gradients = []
                val_gradients = []
                for val_images, val_labels in self.valid_loader:
                    val_images, val_labels = val_images.to(self.device), val_labels.to(self.device)
                    model.zero_grad()
                    val_log_probs = model(val_images)
                    if not (len(val_log_probs.shape) == 2 and val_log_probs.shape[0] == 1):
                        val_log_probs = val_log_probs.squeeze()
                    val_loss = self.criterion(val_log_probs, val_labels)
                    val_loss.backward()
                    val_grad = [param.grad.detach().clone() for param in model.parameters()]
                    val_gradients.append(val_grad)
                    train_gradients.append(train_grad)


                # Calculate JSD between gradients
                jsd_loss = 0.0
                for train_grad, val_grads in zip(train_gradients, val_gradients):
                    for t_g, v_g in zip(train_grad, val_grads):
                        jsd_loss += self.jensen_shannon_divergence(t_g, v_g)

                # Add JSD loss to the main loss
                loss += jsd_loss * self.args.jsd_weight
    ...
    def jensen_shannon_divergence(self, p, q):
        # Calculate the JSD between two gradients p and q

        m = 0.5 * (p + q)
        jsd = 0.5 * (F.kl_div(p, m) + F.kl_div(q[0], m))
        return jsd
...
def train_model(model,
    ...
    with torch.no_grad():
        ...
        for inputs, labels in test_loader:
            ...
            if not (len(test_outputs.shape) == 2 and test_outputs.shape[0] == 1):
