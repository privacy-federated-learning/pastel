...
class CDPLocalUpdate(object):
    ...
    def update_weights(self, model, global_round, model_replacement=False, attack=None):
        ...
        sigma = compute_noise(len(self.train_loader.dataset), self.args.batch_size, args.eps, args.E * args.tot_T, 0.00001, 0.1)
        privacy_engine = PrivacyEngine(
            module=model,
            batch_size=self.args.batch_size,
            sample_size=len(self.train_loader.dataset),
            noise_multiplier=sigma,
            max_grad_norm=self.args.clip)
        ...
        if model_replacement:
            ...
            return state_dict, sum(epoch_loss) / len(epoch_loss), accuracies, epoch_loss
