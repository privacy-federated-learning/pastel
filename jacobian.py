...
def extend(model, input_shape):
    ...
    with torch.no_grad():
        for module in [model.features[0], model.features[2], model.features[4], model.features[6]]:
            y = module(x)
    ...
    def jacobian(self, shape, as_tuple=False):
        ...
        jac = []
        layer = 0
        for module in [model.features[0], model.features[2], model.features[4], model.features[6], model.classifier]:
            if sum(p.numel() for p in module.parameters()):
                try:
